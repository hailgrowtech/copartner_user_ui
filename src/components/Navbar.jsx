...
import { logo, menu, hamburgerBg, LogOut, close, login, userImg } from "../assets";
...
const Navbar = ({ token }) => {
  ...
  const shouldHideToggle = location.pathname.includes("/ra-detail2");
  const logoToShow = shouldHideToggle ? logo : logo;
  const logoDimensions = shouldHideToggle ? { width: "153px", height: "39px" } : { width: "153px", height: "39px" };
        ...
        <div className="fixed top-0 z-50 w-full" style={{ zIndex: 51 }}>
          <div className="" style={{ width: '100vw', margin: '0 -50vw', left: '50%', position: 'relative' }}>
            <Marquee gradient={false} style={{ background: 'linear-gradient(90.81deg, #0181F1 -6.15%, #5ABEE2 54.57%, #AF53AC 107.78%)', color: 'white', padding: '5px 0' }}>
              <p className='px-30'>🥳 BUMPER SALE ON PREMIUM GROUPS 🥳 Special Offer!</p>
              <p className='px-40'>🥳 BUMPER SALE ON PREMIUM GROUPS 🥳 Special Offer!</p>
              <p className='px-40'>🥳 BUMPER SALE ON PREMIUM GROUPS 🥳 Special Offer!</p>
              <p className='px-40'>🥳 BUMPER SALE ON PREMIUM GROUPS 🥳 Special Offer!</p>
            </Marquee>
          </div>
