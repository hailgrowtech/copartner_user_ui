...
const WebinarTabs = () => {
  ...
  return (
    <div className="w-full py-3">
      <div className="flex justify-between bg-[#18181B] items-center p-3 rounded-xl mb-4">
        <div className="flex space-x-4">
          <button
            className={`py-2 px-4 transition-colors duration-300 ${activeTab === 'tab1' ? 'rounded-md text-[#000] bg-[#fff]' : 'text-gray-500 hover:text-[#fff]'}`}
            onClick={() => setActiveTab('tab1')}
          >
            All
          </button>
          <button
            className={`py-2 px-4 transition-colors duration-300 ${activeTab === 'tab2' ? 'rounded-md text-[#000] bg-[#fff]' : 'text-gray-500 hover:text-[#fff]'}`}
            onClick={() => setActiveTab('tab2')}
          >
            Future & Options
          </button>
          <button
            className={`py-2 px-4 transition-colors duration-300 ${activeTab === 'tab3' ? 'rounded-md text-[#000] bg-[#fff]' : 'text-gray-500 hover:text-[#fff]'}`}
            onClick={() => setActiveTab('tab3')}
          >
            Commodity
          </button>
          <button
            className={`py-2 px-4 transition-colors duration-300 ${activeTab === 'tab4' ? 'rounded-md text-[#000] bg-[#fff]' : 'text-gray-500 hover:text-[#fff]'}`}
        ...
        <div className="relative">
          <select
            ...
            className="border rounded-lg py-2 bg-black border-[#ffffff27] px-4 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option value="" disabled className="text-gray-400">Select an option</option>
            <option value="price" className="text-[#fff]">Price</option>
            <option value="latest" className="text-[#fff]">Latest</option>
            <option value="oldest" className="text-[#fff]">Oldest</option>
          </select>
        </div>
      </div>
                  ...
                  <div className="md:p-4 relative z-10 w-full">
                    ...
                    <div className="border-white mt-2 pt-1 flex justify-center">
                        <div className="border-r border-white px-4 lg:px-12 text-center">
                          ...
                          <p className="mb-1 text-gray-300">{course.experience}+</p>
                        </div>
                        <div className="px-4 lg:px-12 text-center">
                          <p className="text-gray-300">Followers</p>
                          <p className="mb-1 text-gray-300">{course.followers}k</p>
                        </div>
                      </div>
                    <div className="mt-2 pt-3 flex flex-col justify-center text-left">
                      <div className="flex flex-row">
                        <p className="mb-1 text-gray-300 pr-1">Date & Time:</p>
                        <p>{course.date}🔴</p>
                      </div>
                      <div className="flex flex-row">
                        <p className="mb-1 text-gray-300 pr-1">Duration:</p>
                    ...
                    <div className="mt-4 pt-3 flex justify-between items-center">
                      <div>
                        <p className="text-gray-300">
                          <span className="text-white text-4xl font-bold">₹1999</span>{" "}
                        </p>
                      </div>
                      <div>
